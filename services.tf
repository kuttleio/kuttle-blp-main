  ...
  services = {
    for service_name, service_config in var.services : service_name => merge(
      local.default_common_settings,
      {
        ...
        environment          = service_config.environment != null && length(service_config.environment) > 0 ? concat(tolist(local.default_common_settings.environment), service_config.environment) : tolist(local.default_common_settings.environment)
