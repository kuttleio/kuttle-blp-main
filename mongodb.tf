locals {
  ...
  standard_tags = merge(var.standard_tags, tomap({
    Name    = "MongoDB Atlas"
    Service = "MongoDB Atlas"
  }))
  connection_bits = {
    for k, v in mongodbatlas_cluster.clusters : k => v.connection_strings[0].standard_srv
  }
}
...
locals {
  ...
  mongodb_clusters = { for item in flatten([
    for project_name, project in var.mongodb_atlas : [
...
resource "mongodbatlas_project" "projects" {
  for_each = var.mongodb_atlas
  name     = "${local.name_prefix}-${var.clp_zenv}-${each.value.project_name}"
...
resource "mongodbatlas_cluster" "clusters" {
  ...
  name                         = "${local.name_prefix}-${var.clp_zenv}-${each.value.project_name}-${each.value.cluster_name}"
...
resource "mongodbatlas_project_ip_access_list" "vpc_nat_ips" {
  ...
  cidr_block = "${var.nat_public_ips[0]}/32"
  comment    = "NAT Gateway IP from AWS VPC"
}

locals {
  mongodb_project_vpn_ip = merge([
    for project_name in keys(var.mongodb_atlas) : {
      for cidr in var.vpn_ip_list : "${project_name}|${cidr}" => cidr
    }
  ]...)
}
...
resource "mongodbatlas_database_user" "users" {
  ...
  username           = "${local.name_prefix}-${var.clp_zenv}-${each.value.username}"
  ...
  roles {
    role_name       = "readWrite"
    database_name   = "${local.name_prefix}-${var.clp_zenv}"
    collection_name = "${local.name_prefix}-${var.clp_zenv}"
  }
}
